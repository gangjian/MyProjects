...
namespace Mr.Robot
{
	...
	public partial class InOutAnalysis
	{
		...
		static void RightValProcess(List<MEANING_GROUP> rightList, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
		...
		static void CalledFunctionProcess(MEANING_GROUP mg, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
		...
		{
			// 在上下文中找出该变量
			VAR_CONTEXT var_ctx = GetVarCtxByName(api.varName, parse_info, func_ctx);
			if (null != var_ctx)
			{
		...
		static List<MEANING_GROUP> GetActualParameterList(MEANING_GROUP mg, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
		...
		// 判断实参的种别(传值或者传引用)
		static ACTUAL_PARA_INFO GetActualParaInfo(MEANING_GROUP act_para, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
		...
		/// 判断变量的类型是值类型亦或是指针类型
		/// </summary>
		static ActParaPassType JudgeVarParaType(string var_name, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
		...
		static VAR_TYPE GetVarType(string var_name, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
			// TODO: 引数?

			// 临时变量?
			foreach (VAR_CONTEXT local_var in func_ctx.LocalVarList)
			{
			...
			// 全局变量?
			VAR_CONTEXT varCtx = parse_info.FindGlobalVarInfoByName(var_name);
			if (null != varCtx)
			{
				return varCtx.Type;
			}
		...
		/// </summary>
		static void CheckRightVarReadOut(MEANING_GROUP rightVal, FILE_PARSE_INFO parse_info, FUNCTION_ANALYSIS_CONTEXT func_ctx)
		{
			...
			VAR_CONTEXT varCtx = GetVarCtxByName(rVarName, parse_info, func_ctx);
			if (null != varCtx)
			{
